...
import resolveSite from './SitesResolver';
...
export default class RouteHandler extends Component {
   ...
   */
  updateLanguage() {
    const newLanguage = resolveSite(this.props.route.match.params.lang).language || this.state.defaultLanguage;

    if (i18n.language !== newLanguage) {
      this.languageIsChanging = true;
      const cookies = new Cookies();
      cookies.set('weareyou-default-lang', this.props.route.match.params.lang, { path: '/', secure: true });
...
function getRouteData(route, language) {
    var site = resolveSite(language);
    const fetchOptions = {
    layoutServiceConfig: { host: config.sitecoreApiHost },
    querystringParams: { sc_lang: site.language, sc_site: site.name, sc_apikey: config.sitecoreApiKey },
    fetcher: dataFetcher,
  };
