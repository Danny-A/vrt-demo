...
const componentRootPath = 'src/components/Sections';
...
function generateComponentFactory() {
  ...
  const imports = [];
  const registrations = [];
  const closingImports = [];

  // Import global stylesheets
  imports.push(`import '../sass/leading.scss';`);
  imports.push(`import '../sass/engine.scss';`);
  // Import closing global stylesheets
  closingImports.push(`import '../sass/closing.scss';`);

  fs.readdirSync(componentRootPath).forEach((componentFolder) => {
    if (!fs.existsSync(path.join(componentRootPath, componentFolder, 'index.jsx'))) return;

    const importVarName = componentFolder.replace(/[^\w]+/g, '');

    console.debug(`Registering JSS component ${componentFolder}`);
    imports.push(`import ${importVarName} from '../components/Sections/${componentFolder}';`);
